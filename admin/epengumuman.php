<?php 
	...
	if(isset($_POST["simpan"])){
		...
		$editfile = $_FILES['editfile'] ['name'];
        $ukuran_file = $_FILES['editfile']['size'];
        $tipe_file = pathinfo($editfile,PATHINFO_EXTENSION);
        $random = md5(uniqid($editfile, true) . time());
        $tmp_file = $_FILES['editfile']['tmp_name'];
        $path = "file/".$random.'.'.$tipe_file;

		if(empty($editfile)){
					$query = "update pengumuman set judul='$editjudul' where id='$idp'";
	      ...
	      if($tipe_file == 'pdf'){ 
	        if($ukuran_file <= 5000000){ 
	          move_uploaded_file($tmp_file, $path);	
					$query = "update pengumuman set judul='$editjudul', fpgm='$path' where id='$idp'";
					$queryrun = mysqli_query($conn, $query);
						if(!$queryrun){
				            die ("Query gagal dijalankan: ".mysqli_errno($conn).
				                " - ".mysqli_error($conn));
					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								...
								<div class="formulir">
									<form action="epengumuman.php" method="post" enctype="multipart/form-data" >
										<div class="form-group">
											<label>Judul</label>
											<input name="editjudul" type="text" class="form-control" value="<?php echo $p['judul'] ?>" required>
										</div>
										<div class="form-group col-4">
											<label>Upload File Pengumuman (PDF)</label>
											<input name="editfile" type="file" class="form-control" value="<?php echo $fpgm ?>">
										</div>
										<div>
